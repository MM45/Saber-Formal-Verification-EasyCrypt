...
proof.
(* Introduce useful assumption with have *)
have eq_pr: Pr[Game1(A).main() @ &m : res] = Pr[Game2( A2(A) ).main() @ &m : res].

  (* Proof of assumption introduced by have *)
  byequiv; trivial.
  proc.
  inline A2(A).choose; inline A2(A).guess.
  wp.
  call (_ : true).
  auto.
  call (_ : true).
  ...
  congr; congr.
    ...
    (* Proof equality first (output) tuple element *)
    rewrite cg_enc_dec_inv. simplify.
    rewrite scaleRp2Rppq2R2t_comp.
    trivial.
    
    (* Proof equality second (output) tuple element *)
    rewrite cg_enc_dec_inv.
    trivial.

(* Proof of original claim using assumption introduced by initial have *)
rewrite eq_pr.
trivial.
qed.
