...
-----------------------------------
*)
require import AllCore Bool Distr DBool ZModP IntDiv StdOrder.
...
proof.
  ...
  by rewrite mulzC -(intmulz ep 2) mulr2z -(lez_add2r (-1) _ _) /= -(lez_add2l ((-(ep + ep))) _ _) addzA /= addzC opprD addzA.
qed.  
...
proof.
  ...
  by rewrite -mul1r2z mulrC /ofint_id intmulz /= (lez_trans (eq + 1) _ _); [rewrite (lez_addl eq 1) | apply geeq1_2ep].
qed.
...
proof.
  ...
  apply IntOrder.ler_weexpn2l; split; [by apply (lez_trans eq 0 (eq + 1)); [apply /(lez_trans 3 0 eq) /ge3_eq | rewrite (lez_addl eq 1)] |].
...
proof.
  ...
  have ->: p * p %/ q * q = p * p.
   by rewrite -(dvdz_eq q (p * p)) q_div_pp.
  rewrite /p /q -exprD_nneg; first by rewrite subz_ge0 (lez_trans (eq + 1) eq (2 * ep)); [rewrite (lez_addl _ 1) | apply geeq1_2ep].
...
proof. 
  ...
  apply /(scale_split (Zp.asint x) ep (2 * ep - eq) (et + 1)) /ge2epeq_ep; 
...
proof.
  have ->: Pr[Game1(A).main() @ &m : res] = Pr[Game2( A2(A) ).main() @ &m : res].
   ...
   by progress; do! congr; rewrite c_enc_dec_inv; first rewrite scale_split_p_ppq_2t.  
  by reflexivity.
qed.
